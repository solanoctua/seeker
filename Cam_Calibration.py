...
It makes sense: they are intrinsic parameters of a camera and they don't depend on particular images which you show to the camera.
However, poses of the camera do depend on square size.
Again it makes sense: if a chessboard filled the whole image but square size is very small then it means the camera is very close to the pattern. 
But if square size is large then it means the camera is farther from the pattern.
if you want to get just intrinsics (camera matrix and distortion) it doesn't matter which square size you specify (you can use just the default value).
If you want to get extrinsics (rvecs and tvecs, that is poses of a camera) then you should specify accurate square size to get valid results.
author:ilya-lysenkov (https://twitter.com/ilysenkov)

that is, line 111 is important but line 34 is not for our purposes
"""
...
"""
file = cv2.FileStorage(path_of_images+"/calibration_data.txt", cv2.FILE_STORAGE_WRITE)
file.write("mtx", mtx)
